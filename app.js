document.addEventListener('DOMContentLoaded', async () => {
    ...
    const holidays = {
        ...
        // 2026 추석은 목·금·토 (9/24~26) — 일요일과 겹치는 날 없어 대체공휴일 없음
        ...
        "2027-09-16": "추석연휴",
        // 2027 대체공휴일
        "2027-02-09": "대체공휴일",   // 설날(일) → 연휴 다음 평일인 화
        "2027-05-03": "대체공휴일",   // 근로자의 날(토) → 월. 근로자의 날은 2026년 공휴일 지정
        "2027-07-19": "대체공휴일",   // 제헌절(토) → 월. 제헌절은 2026년 공휴일 재지정
        "2027-08-16": "대체공휴일",   // 광복절(일) → 월
        "2027-10-04": "대체공휴일",   // 개천절(일) → 월
        "2027-10-11": "대체공휴일",   // 한글날(토) → 10/4가 이미 공휴일이라 그 다음 월
        "2027-12-27": "대체공휴일",   // 성탄절(토) → 월
        // 2027 추석은 화, 수, 목 (9/14~16) — 주말과 겹치지 않아 대체공휴일 없음
        // 2027 현충일(6/6 일)은 규정상 대체공휴일 대상 아님
        // 2028 Lunar
        "2028-01-26": "설연휴",
        "2028-01-27": "설날",
        "2028-01-28": "설연휴",
        "2028-05-02": "부처님오신날",
        "2028-10-02": "추석연휴",
        "2028-10-03": "추석/개천절",   // 추석과 개천절이 같은 날
        "2028-10-04": "추석연휴",
        // 2028 대체공휴일
        "2028-10-05": "대체공휴일",   // 추석(10/3)이 개천절과 겹쳐 발생. 10/2~10/5 나흘 연휴
        // 2028 설연휴는 수, 목, 금 (1/26~28) — 주말과 겹치지 않아 대체공휴일 없음
        // 2028 신정(1/1 토)은 규정상 대체공휴일 대상 아님
        // 2028 나머지 공휴일은 모두 평일이라 대체공휴일 없음
        "05-01": "근로자의 날"
    };
    ...
    const HARDCODED_WEEKS = {
        '2026-04-06': 'menuContentTable',   // 4월 둘째주
        '2026-04-13': 'menuContentTable2',  // 4월 셋째주
        '2026-04-20': 'menuContentTable3',  // 4월 넷째주
        '2026-04-27': 'menuContentTable4',  // 4월 다섯째주
    ...
    if (sb) {
        ...
        // 일정표 자동 반영 (Drive 폴더 감시 + 자동 추가 적용)
        fetch('/api/schedule_imports/poll', { method: 'POST' })
            .then(r => r.json())
            .then(d => {
                if (d.status === 'ok' && d.processed && d.processed.length) {
                    console.log('✅ 일정표 자동 반영:', d.processed);
                    // 추가된 일정 있으면 다시 로드
                    const hasAdds = d.processed.some(p => (p.applied_adds || 0) > 0);
                    if (hasAdds && typeof loadSchedules === 'function') {
                        loadSchedules().then(() => renderCalendar && renderCalendar());
                    }
                }
            })
            .catch(_ => {});
    ...
    async function syncAutoRequestForMonth(ym) {
        ...
        const offsetItems = [
            { suffix: 'revenue-est',  title: `경영실적(예상) 엑셀자료 회신`,                  cat: '통합회의및확대회의관련', offset: -4 },
            { suffix: 'conf-submit',  title: `확대회의 PPT자료 회신 (NBT/BIO/펫/파마)`,    cat: '통합회의및확대회의관련', offset: -3 },
            { suffix: 'conf-minutes', title: `확대회의 회의록 회신`,                         cat: '통합회의및확대회의관련', offset: 1 },
        ];
